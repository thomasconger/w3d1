...
def substrings(string)
  #for all given possible lengths
    #iterate through string while possible
  #eg, 1 > [c,a,t,s]
  #eg, 2 > [ca, at, ts]
  list = []
  string.each_char.with_index do |ltr, idx|
    (1..string.length).each do |ss_length|
      if idx + ss_length <= string.length
        list << string[idx...idx+ss_length]
      end
    end
  end
  list
...
def subwords(word, dictionary)
  potential_sub_strings = substrings(word)
  potential_sub_strings & dictionary
end
